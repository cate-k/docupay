## DocuPay
### Project Video
The project demo/walkthrough video was even uploaded using DocuPay. [Click here to watch!](https://bafybeibswvettcxxsldpwpvlyzujitwjer4cwb7xsy3mlylwj67u3mma3y.ipfs.infura-ipfs.io/)

##

### Overview

DocuPay is a platform where users can publish their documents and earn ETH when others purchase them.

Currently, users can only upload files to IPFS, and there is no feed where you can find documents posted by others yet. However, I am planning to flesh out this project more in the next few months once I expand my knowledge of Solidity and ReactJS.

It uses IPFS, or the Inter Planetary File System, to upload the files and is deployed on the Rinkeby testnet.

##

### Setting Up
*Note:* This guide assumes that you are on a Windows machine.

**Step #1:** Navigate to the [repository page](https://github.com/cate-k/docupay) and click on the green button labelled *Code*. Check that you are on the *HTTPS* tab and click the clipboard icon to copy the link.

**Step #2:** Open up the command prompt and type in `git clone https://github.com/cate-k/docupay.git`

**Step #3:** Once the project has finished cloning, change into the project's directory.

**Step #4:** Change into the `client` directory and enter `npm i react-scripts --save`.

**Step #5:** Finally, run `npm run start` to open up `localhost`. If it doesn't open in your browser automatically, simply type in `localhost:3000` in the address bar.
