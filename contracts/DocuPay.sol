...
contract DocuPay {
  ...
  function isDocumentPurchased(address user, uint256 docId) public view returns(bool) {
    // Check if the user has purchased the document previously
    for (uint256 i = 0; i < documentsPurchased[user].length; i++) {
      if (documentsPurchased[user][i] == docId)
        return true;
    }
  ...
  function canView(address user, uint256 docId) public view returns(bool) {
    // If the user uploaded the document or they have purchased the document before, they can view it
    if (user == getDocumentById(docId).uploader || isDocumentPurchased(user, docId))
      return true;
